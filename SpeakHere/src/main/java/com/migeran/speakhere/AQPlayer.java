...
public class AQPlayer implements Function_AudioQueueNewOutput, Function_AudioQueueAddPropertyListener {
	...
	public int StartQueue(boolean inResume) {
		...
		if (!inResume) {
			...
			for (int i = 0; i < kNumberBuffers; ++i) {
				call_AudioQueueNewOutput(null, null, mBuffers.get(i).get());
			}
		}
	...
	public void CreateQueueForFile(String inPath) {
		...
		try {
			if (mFilePath == null) {
				...
				CFStringRef inFilePath = CFStringCreateWithCString(kCFAllocatorDefault(), inPath, CFStringBuiltInEncodings.EncodingUTF8);
				sndFile = CFURLCreateWithFileSystemPath(kCFAllocatorDefault(), inFilePath, CFURLPathStyle.POSIXPathStyle, (byte) 0);
				...
				int rc = AudioFileOpenURL(sndFile, (byte) Enums.kAudioFileReadPermission, 0/* inFileTypeHint */, audioFileRef);
	...
	public void call_AudioQueueNewOutput(VoidPtr inUserData, VoidPtr inAQ, AudioQueueBuffer inCompleteAQBuffer) {
		...
		int result = AudioFileReadPackets(getAudioFileID(), FALSE, numBytesRef, inCompleteAQBuffer.mPacketDescriptions(), getCurrentPacket(), nPacketsRef, inCompleteAQBuffer.mAudioData());
		if (result != 0)
